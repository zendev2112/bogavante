...
export default function AdminStockPage() {
  ...
  useEffect(() => {
    ...
    if (sushiData && (sushiData as any).insumos_para_sushi) {
      ...
      Object.entries(sushiInsumos).forEach(
  ...
  const filteredProducts = products.filter((product) => {
    ...
    const matchSearch = product.product
      .toLowerCase()
  ...
  const totalAvailable = products.reduce((total, product) => {
    return (
      total + product.presentations.filter((p) => p.available && p.price).length
    )
  }, 0)
        ...
        <div className="grid gap-4 mb-8">
          {filteredProducts.map((product) => {
            const availableUnits = [
              ...new Set(product.presentations.map((p) => p.unit)),
            ]
            const filteredPresentations = product.presentations.filter(
              (p) => p.unit === product.selectedUnit,
            )

            return (
              <Card
                key={product.id}
                className={`shadow-sm transition-all ${product.inStock ? 'ring-2 ring-blue-300' : ''}`}
              >
                    ...
                    {availableUnits.length > 1 && (
                      <div className="flex flex-col items-start lg:items-end gap-2">
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            changeUnit(product.id, e.target.value)
                          }
                    ...
                    <div className="space-y-3">
                      ...
                      {filteredPresentations.length === 0 && (
                        <p className="text-gray-500 italic">
                          No hay presentaciones disponibles para la unidad
                          seleccionada
                        </p>
                      )}
